...
current = handle.model.fkine(q0);
handle.model.animate(q0)
...
for k = 1:6
    q0 = handle.model.getpos;
    q1 = handle.model.ikcon(transl(positions(k+1,:)),q0);
    x = positions(k,1);
    y = positions(k,2);
    z = positions(k,3);
    ...
    qWaypoints = [q0 ; handle.model.ikcon(transl(x,y,z),q0)];
    [~,all] = handle.model.fkine(qWaypoints(end,:));
    for j = 1:6
        for i = 1 : size(all,3)-1
            for faceIndex1 = 1:size(faces1,1)
                futureWaypoints = [qWaypoints ; handle.model.ikcon(transl(x+fx,y+fy,z+fz),qWaypoints(end,:))];
                [~,newall] = handle.model.fkine(futureWaypoints(end,:));
        ...
        qWaypoints = [qWaypoints; handle.model.ikcon(transl(x,y,z),qWaypoints(end,:))];
        x=x+fx;
        y=y+fy;
        z =z+fz;
        
    end
    qWaypoints = [qWaypoints; handle.model.ikcon(transl(x,y,z),q1)];
    ...
    x1 = handle.model.fkine(q0);
    x2 = handle.model.fkine(qWaypoints(end,:));
    for i = 1:steps
    ...
    for i = 1:steps-1
        T = handle.model.fkine(real(qWaypoints(i,:)));
        ...
        J = handle.model.jacob0(real(qWaypoints(i,:)));
        m(i) = sqrt(det(J*J'));
        ...
        qdot(i,:) = (invJ*xdot)';
        for j = 1:6
            if qWaypoints(i,j) + deltaT*qdot(i,j) < handle.model.qlim(j,1)
                qdot(i,j) = 0; % Stop the motor
            elseif qWaypoints(i,j) + deltaT*qdot(i,j) > handle.model.qlim(j,2)
                qdot(i,j) = 0; % Stop the motor
            end
        end
...
for i=1:10
    ...
    handle.model.animate(q');
    
    %Get camera location
    Tc = handle.model.fkine(q);
    cam.T = Tc;
...
% q1 = handle.model.ikcon(redPos);
% qMatrix = jtraj(q0, q1, steps);
%
% for i = 1:steps
%     handle.model.animate(qMatrix(i,:));
%     drawnow();
% end
%
% q2 = handle.model.ikcon(redGoalPose);
% qMatrix = jtraj(q1, q2, steps);
%
% for i = 1:steps
%     handle.model.animate(qMatrix(i,:));
%     redStart = handle.model.fkine(qMatrix(i,:));
%     delete(redCanLocation);
%     redCanLocation = redCan(redStart*troty(pi/2));
%     drawnow();
% end
...
% % Green Can Sequence
%
% q3 = handle.model.ikcon(greenPos);
...
% end
%
% q4 = handle.model.ikcon(greenGoalPose);
% qMatrix = jtraj(q3, q4, steps);
%
% for i = 1:steps
%     handle.model.animate(qMatrix(i,:));
%     greenStart = handle.model.fkine(qMatrix(i,:));
%     delete(greenCanLocation);
%     greenCanLocation = greenCan(greenStart*troty(pi/2));
%     drawnow();
% end
...
% end
%
% q6 = handle.model.ikcon(blueGoalPose);
% qMatrix = jtraj(q5, q6, steps);
%
% for i = 1:steps
%     handle.model.animate(qMatrix(i,:));
%     blueStart = handle.model.fkine(qMatrix(i,:));
%     delete(blueCanLocation);
%     blueCanLocation = blueCan(blueStart*troty(pi/2));
%     drawnow();
% end
